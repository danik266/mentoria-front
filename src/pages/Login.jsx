    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.detail === 'Email not confirmed') {
          toast.error(t('login.notConfirmed'));
          setTimeout(() => {
            navigate('/confirm-email', { state: { email } });
          }, 2000);
        } else {
          toast.error(data.detail || t('login.failed'));
        }
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="text-center text-3xl font-extrabold text-slate-900 dark:text-white">{t('login.title')}</h2>
