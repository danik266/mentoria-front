...
export default function OpportunityCard({ op, onDetails, onToggleSave }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col">
      ...
      <h3 className="font-bold text-slate-800 text-lg leading-snug mb-2">{op.title}</h3>

      <div className={`flex items-center gap-1.5 text-sm font-medium mb-3 ${urgent ? 'text-red-600' : 'text-slate-500'}`}>
        <Icon name="schedule" className="text-[18px]" />
        <span>
          до {formatDate(op.deadline)}
          {urgent && days >= 0 && ` · осталось ${days} дн.`}
          {days < 0 && ' · завершено'}
        </span>
      </div>

      <p className="text-sm text-slate-600 mb-3 line-clamp-3 flex-1">{op.description}</p>
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 mb-4">
        <span className="inline-flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-md">
          <Icon name="groups" className="text-[16px]" /> {op.grades.join(', ')} класс
        </span>
        <span className="inline-flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-md">
          <Icon name="public" className="text-[16px]" /> {op.format}
        </span>
      </div>

      <div className="text-xs text-slate-500 mb-4">
        <span className="font-semibold text-slate-600">Требования:</span> {op.requirements}
      </div>
