    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">Читай внимательно — это основа урока</p>
        </div>
      </div>
          ...
          if (block.type === 'formula') {
            return (
              <div key={i} className="bg-slate-900 rounded-2xl p-5 text-center">
                <p className="text-xs text-slate-400 mb-2 uppercase tracking-wide">{block.label || 'Формула'}</p>
                <p className="text-2xl font-bold text-white font-mono">{block.text}</p>
                {block.desc && <p className="text-slate-400 text-sm mt-2">{block.desc}</p>}
              </div>
            )
          }
          if (block.type === 'list') {
            return (
              <div key={i} className="bg-white border border-slate-100 rounded-2xl p-5">
                {block.title && <p className="font-semibold text-slate-800 mb-3">{block.title}</p>}
